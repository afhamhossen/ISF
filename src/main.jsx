...
function App(){
 ...
 async function createBusiness(e){e.preventDefault();if(!newBizName.trim())return setMsg("Enter a business name.");const {data,error}=await supabase.from("businesses").insert({name:newBizName.trim(),currency:newBizCurrency,created_by:session.user.id}).select().single();setMsg(error?.message||"Business created.");if(!error){setNewBizName("");await loadBusinesses();setBusinessId(data.id);if(typeof localStorage!=="undefined")localStorage.setItem("isf_business_id",data.id);loadData(data.id)}}
 async function updateBusinessInfo(e){e.preventDefault();if(!canAdmin)return setMsg("Permission denied.");const {error}=await supabase.from("businesses").update({name:bizNameEdit.trim(),currency:bizCurrencyEdit}).eq("id",businessId);if(!error)await audit("UPDATE","businesses",businessId,"business info updated");setMsg(error?.message||"Business updated.");if(!error)loadBusinesses()}
 ...
 async function addPerson(e){e.preventDefault();if(!allowed("admin","manager"))return setMsg("Permission denied.");const table=person.type==="member"?"members":"agents";const {data,error}=await supabase.from(table).insert({business_id:businessId,name:person.name.trim(),phone:person.phone.trim(),active:true}).select().single();if(!error)await audit("CREATE",table,data.id,person.name);setMsg(error?.message||"Added.");if(!error){setPerson({...person,name:"",phone:""});loadData(businessId)}}
 async function togglePerson(p){if(!allowed("admin","manager"))return setMsg("Permission denied.");const table=p.person_type==="member"?"members":"agents";const {error}=await supabase.from(table).update({active:!p.active}).eq("id",p.id);if(!error)await audit("UPDATE",table,p.id,`active -> ${!p.active}`);setMsg(error?.message||"Updated.");if(!error)loadData(businessId)}
 ...
 async function saveTx(e){e.preventDefault();if(!allowed("admin","manager","member","agent"))return setMsg("Permission denied.");if(!form.person_id||!+form.amount)return setMsg("Select person and amount.");
  ...
  if(editing){({error}=await supabase.from("transactions").update(payload).eq("id",editing));if(!error)await audit("UPDATE","transactions",editing,JSON.stringify(payload));setMsg(error?.message||"Transaction updated.");setEditing(null)}
  else {({data,error}=await supabase.from("transactions").insert({...payload,created_by:session.user.id}).select().single());if(!error)await audit("CREATE","transactions",data.id,JSON.stringify(payload));setMsg(error?.message||"Transaction saved.")}
 ...
 async function deleteTx(id){if(!allowed("admin","manager"))return setMsg("Only Admin/Manager can delete.");if(!confirm("Delete this transaction?"))return;const {error}=await supabase.from("transactions").delete().eq("id",id);if(!error)await audit("DELETE","transactions",id,"Transaction deleted");setMsg(error?.message||"Transaction deleted.");if(!error)loadData(businessId)}
 async function addOpening(e){e.preventDefault();if(!allowed("admin","manager"))return setMsg("Permission denied.");const {data,error}=await supabase.from("opening_balances").insert({...opening,amount:+opening.amount,business_id:businessId,created_by:session.user.id}).select().single();if(!error)await audit("CREATE","opening_balances",data.id,JSON.stringify(opening));setMsg(error?.message||"Opening balance saved.");if(!error){setOpening({...opening,amount:"",note:""});loadData(businessId)}}
 async function dailyClose(){if(!allowed("admin","manager","super_admin"))return setMsg("Only authorized users can close a day.");const net=dayStats.collection-dayStats.fund-dayStats.expense;const {data,error}=await supabase.from("daily_closings").upsert({business_id:businessId,closing_date:closeDate,total_fund:dayStats.fund,total_collection:dayStats.collection,total_expense:dayStats.expense,net_result:net,closed_by:session.user.id},{onConflict:"business_id,closing_date"}).select().single();if(!error)await audit("CLOSE","daily_closings",data.id,closeDate);setMsg(error?.message||"Daily closing saved.");if(!error)loadData(businessId)}
 function personLedger(id,date=""){const rows=tx.filter(t=>t.person_id===id&&(!date||t.transaction_date===date));return sumTx(rows)}

 async function addMemberByEmail(e){e.preventDefault();if(!canAdmin)return setMsg("Permission denied.");const email=addMemberEmail.trim();if(!email)return;const {data:uid,error:e1}=await supabase.rpc("find_user_id_by_email",{lookup_email:email});if(e1||!uid)return setMsg("No account found for that email yet — ask them to sign in with Google once first, then try again.");const {error}=await supabase.from("business_members").insert({business_id:businessId,user_id:uid,role:"pending"});if(!error)await audit("CREATE","business_members",uid,`added ${email}`);setMsg(error?.message||"User added — set their role below.");if(!error){setAddMemberEmail("");loadData(businessId)}}
 async function updateMemberRole(memberRowId,userId,role){if(!canAdmin)return setMsg("Permission denied.");const {error}=await supabase.from("business_members").update({role}).eq("id",memberRowId);if(!error)await audit("UPDATE","business_members",userId,`role -> ${role}`);setMsg(error?.message||"Updated.");if(!error)loadData(businessId)}
 async function removeMember(memberRowId,userId){if(!canAdmin)return setMsg("Permission denied.");if(!confirm("Remove this user from the business?"))return;const {error}=await supabase.from("business_members").delete().eq("id",memberRowId);if(!error)await audit("DELETE","business_members",userId,"removed from business");setMsg(error?.message||"Removed.");if(!error)loadData(businessId)}
 async function addChannel(e){e.preventDefault();if(!canManage)return setMsg("Permission denied.");const nm=newChannel.trim();if(!nm)return setMsg("Enter a channel name.");const {data,error}=await supabase.from("channels").insert({business_id:businessId,name:nm}).select().single();if(!error)await audit("CREATE","channels",data.id,nm);setMsg(error?.message||"Channel added.");if(!error){setNewChannel("");loadData(businessId)}}
 async function toggleChannel(c){if(!canManage)return setMsg("Permission denied.");const {error}=await supabase.from("channels").update({active:!c.active}).eq("id",c.id);if(!error)await audit("UPDATE","channels",c.id,`active -> ${!c.active}`);setMsg(error?.message||"Channel updated.");if(!error)loadData(businessId)}
