...
import "./styles.css";
import CashBookPro from "./cashbookpro.jsx";
...
// Quick date-range presets shared by the transaction and cash-book filter bars.
const datePresets=["All Time","Today","Yesterday","This Month","Last Month"];
...
// Small pill-style date-range picker: tapping a preset fills dateFrom/dateTo for you;
// the two date inputs stay editable for a custom Single Day / Date Range choice.
// onChange receives a single patch object so callers can merge it into their filter state in one setState call.
function DateFilterChips({preset,dateFrom,dateTo,onChange}){
 return <div className="filter-chips">
  {datePresets.map(p=><button type="button" key={p} className={"chip"+(preset===p?" chip-active":"")} onClick={()=>onChange({preset:p,...presetRange(p)})}>{p}</button>)}
  <button type="button" className={"chip"+(preset===""?" chip-active":"")} onClick={()=>onChange({preset:""})}>Date Range</button>
  {preset===""&&<>
...
function App(){
 ...
 const [opening,setOpening]=useState({date:today(),channel:"Cash",amount:"",note:""}),[editing,setEditing]=useState(null),[filter,setFilter]=useState({preset:"All Time",dateFrom:"",dateTo:"",person:"",channel:"",type:""});
 // Duplicate Business: create a new business and optionally copy this one's members/agents and payment channels.
 const [dupOpen,setDupOpen]=useState(false),[dupForm,setDupForm]=useState({name:"",copyMembers:true,copyChannels:true});
 ...
 useEffect(()=>{if(session&&(appMode==="business"||appMode==="cashbookpro"))loadBusinesses()},[session,appMode]);
 ...
 // Create a new business with the same currency, then optionally copy this business's
 // payment channels and members/agents into it. Transactions/history are never copied.
 async function duplicateBusiness(e){
  e.preventDefault();if(!dupForm.name.trim())return setMsg("Enter a name for the new business.");
  const {data,error}=await supabase.from("businesses").insert({name:dupForm.name.trim(),currency:currentBiz?.currency||"BDT",created_by:session.user.id}).select().single();
  if(error)return setMsg(friendly(error.message));
  const newId=data.id;
  if(dupForm.copyChannels){
   await supabase.from("channels").delete().eq("business_id",newId);
   if(channelsDb.length)await supabase.from("channels").insert(channelsDb.map(c=>({business_id:newId,name:c.name,active:c.active,sort_order:c.sort_order})));
  }
  if(dupForm.copyMembers){
   if(members.length)await supabase.from("members").insert(members.map(m=>({business_id:newId,name:m.name,phone:m.phone,active:m.active})));
   if(agents.length)await supabase.from("agents").insert(agents.map(a=>({business_id:newId,name:a.name,phone:a.phone,active:a.active})));
  }
  setMsg(`Business "${dupForm.name.trim()}" created from ${currentBiz?.name||"this business"}.`);
  setDupForm({name:"",copyMembers:true,copyChannels:true});setDupOpen(false);
  await loadBusinesses();setBusinessId(newId);if(typeof localStorage!=="undefined")localStorage.setItem("isf_business_id",newId);loadData(newId);loadV8(newId);
 }
 ...
 if(appMode==="cashbookpro")return <CashBookPro supabase={supabase} session={session} profile={profile} businesses={businesses} businessId={businessId} setBusinessId={switchBusiness} onExit={()=>setAppMode("")}/>;
...
function CashBookApp({entries,form,setForm,submit,editing,edit,del,cancel,totals,email,logout,back,msg,setMsg,cbTab,setCbTab,contacts,contactForm,setContactForm,saveContact,deleteContact,ledgerEntries,ledgerForm,setLedgerForm,saveLedgerEntry,ledgerEditing,editLedger,cancelLedger,deleteLedgerEntry,contactBalance,sendReminder,exportExcel,exportPDF,backup,
 ...
 // Entries-tab filters (date preset/range, type, category, payment mode). Kept local to this
 // component since it only affects what's shown here, not the totals used by PDF/Excel/Backup.
 const [cbFilter,setCbFilter]=useState({preset:"All Time",dateFrom:"",dateTo:"",type:"",category:"",channel:""});
 const cbCategories=useMemo(()=>[...new Set(entries.map(x=>x.category).filter(Boolean))],[entries]);
 const filteredEntries=useMemo(()=>entries.filter(x=>(!cbFilter.dateFrom||x.entry_date>=cbFilter.dateFrom)&&(!cbFilter.dateTo||x.entry_date<=cbFilter.dateTo)&&(!cbFilter.type||x.type===cbFilter.type)&&(!cbFilter.category||x.category===cbFilter.category)&&(!cbFilter.channel||x.channel===cbFilter.channel)),[entries,cbFilter]);
 const filteredTotals=useMemo(()=>filteredEntries.reduce((a,x)=>{const v=+x.amount||0;if(x.type==="in")a.in+=v;else a.out+=v;return a},{in:0,out:0}),[filteredEntries]);
 const cbActiveCount=(cbFilter.preset&&cbFilter.preset!=="All Time"?1:0)+["type","category","channel"].filter(k=>cbFilter[k]).length;
 const cbClearFilters=()=>setCbFilter({preset:"All Time",dateFrom:"",dateTo:"",type:"",category:"",channel:""});
 ...
 {cbTab==="entries"&&<>
  <div className="stats"><Stat title="Total Cash In" value={filteredTotals.in} fmt={cbFmt}/><Stat title="Total Cash Out" value={filteredTotals.out} fmt={cbFmt}/><Stat title="Balance" value={filteredTotals.in-filteredTotals.out} fmt={cbFmt}/></div>
  <div className="card">
   <h3>Filters{cbActiveCount>0&&<span className="badge active"> {cbActiveCount}</span>}</h3>
   <DateFilterChips preset={cbFilter.preset} dateFrom={cbFilter.dateFrom} dateTo={cbFilter.dateTo} onChange={patch=>setCbFilter({...cbFilter,...patch})}/>
   <div className="form">
    <label>Entry Type<select value={cbFilter.type} onChange={e=>setCbFilter({...cbFilter,type:e.target.value})}><option value="">All</option><option value="in">Cash In</option><option value="out">Cash Out</option></select></label>
    <label>Category<select value={cbFilter.category} onChange={e=>setCbFilter({...cbFilter,category:e.target.value})}><option value="">All</option>{cbCategories.map(c=><option key={c}>{c}</option>)}</select></label>
    <label>Payment Mode<select value={cbFilter.channel} onChange={e=>setCbFilter({...cbFilter,channel:e.target.value})}><option value="">All</option>{defaultChannels.map(c=><option key={c}>{c}</option>)}</select></label>
   </div>
   <div className="actions"><button onClick={cbClearFilters}>✕ Clear all</button></div>
  </div>
  ...
  <div className="card"><div className="muted">Showing {filteredEntries.length} of {entries.length} entries</div><div className="table-wrap"><table><thead><tr><th>Date</th><th>Type</th><th>Category</th><th>Channel</th><th>Amount</th><th>Note</th><th>Action</th></tr></thead><tbody>{filteredEntries.map(x=><tr key={x.id}><td>{x.entry_date}</td><td>{x.type==="in"?"Cash In":"Cash Out"}</td><td>{x.category}</td><td>{x.channel}</td><td>{cbFmt(x.amount)}</td><td>{x.note||""}</td><td><button onClick={()=>edit(x)}>Edit</button><button className="danger" onClick={()=>del(x.id)}>Delete</button></td></tr>)}</tbody></table></div>{!filteredEntries.length&&<p className="muted">No entries found for the current filters. <button onClick={cbClearFilters}>Clear filters</button></p>}</div>
 </>}
 {cbTab==="khata"&&<>
...
function Reports({tx,allTx,name,pdf,excel,filter,setFilter,edit,del,people,allChannelNames,fmt}){
 const clearAll=()=>setFilter({preset:"All Time",dateFrom:"",dateTo:"",person:"",channel:"",type:""});
 const activeCount=["dateFrom","person","channel","type"].filter(k=>filter[k]&&!(k==="dateFrom"&&filter.preset==="All Time")).length;
 return <section><div className="section-head"><div><h1>Reports</h1><p className="muted">Search, edit, delete and export.</p></div><div className="actions"><button onClick={pdf}>PDF</button><button onClick={excel}>Excel</button></div></div>
 <div className="card">
  <h3>Filters{activeCount>0&&<span className="badge active"> {activeCount}</span>}</h3>
  <DateFilterChips preset={filter.preset} dateFrom={filter.dateFrom} dateTo={filter.dateTo} onChange={patch=>setFilter({...filter,...patch})}/>
  <div className="form">
   <label>Entry Type<select value={filter.type} onChange={e=>setFilter({...filter,type:e.target.value})}><option value="">All</option><option value="collection">Collection</option><option value="fund_given">Fund Given</option><option value="expense">Expense</option></select></label>
   <label>Members<select value={filter.person} onChange={e=>setFilter({...filter,person:e.target.value})}><option value="">All</option>{people.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
   <label>Payment Mode<select value={filter.channel} onChange={e=>setFilter({...filter,channel:e.target.value})}><option value="">All</option>{allChannelNames.map(c=><option key={c}>{c}</option>)}</select></label>
  </div>
  <div className="actions"><button onClick={clearAll}>✕ Clear all</button></div>
 </div>
 <div className="card"><div className="muted">Showing {tx.length} of {allTx.length} transactions</div><Table tx={tx} name={name} edit={edit} del={del} fmt={fmt}/></div></section>}
